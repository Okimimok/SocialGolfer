...
class TestDPHonhon(unittest.TestCase):
	def setUp(self):
		...
		self.f	   = [0.0, 0.2, 0.4, 0.6, 0.8]
		self.custs = [[i, 4, 0.2] for i in xrange(5)]
	...
	def test_FirstStep(self):	
		...
		SOpt = DP(self.tree, self.rev, np.zeros(5), self.f, self.custs)[0]
	...
	def test_LowerBd(self):
		...
		tmp = DeltaMinus(self.tree, self.rev, [], 4, self.f, self.K, self.custs)
		self.assertAlmostEqual(tmp, -1.56)
	...
	def test_UpperBd(self):
		...
		tmp = DeltaPlus(self.tree, self.rev, [], 4, self.f, self.K, self.custs)
		self.assertAlmostEqual(tmp, 0.20)

	def test_solve(self):
		# Honhon's algorithm, from start to finish. Optimal assortment is {2, 3}.
		#	Revenue of 6.78. (Actually, 6.72, but we tweak Honhon's model slightly.)
		opt, optObj = Alg3(self.tree, self.rev, self.K, self.f, self.custs)
		self.assertAlmostEqual(optObj, 6.78)
		self.assertEqual(sorted(opt), [2, 3])

	def test_optRev(self):
		# Checking if optimal assortment yields the desired revenue
		tmp = NPrev(self.tree, self.custs, self.rev, self.K, self.f, [2, 3])
		self.assertAlmostEqual(tmp, 6.78)
	...
	def test_purchase(self):
		...
		buy  = [0, 0, 0, 0, 0]		
		for i in xrange(5):
			path = self.tree.get_path(self.custs[i][0], self.custs[i][1])
...
class TestDPSingleton2(unittest.TestCase):
	...
	def test_FirstStep(self):	
		...
		SOpt1 = DP(self.tree, self.rev, [0], self.f, self.custs)[0]
		self.assertEqual(SOpt1, [0])

	def test_LowerBd(self):
		# Lower bound and upper bound should both be -1.
		tmp = DeltaMinus(self.tree, self.rev, [], 0, self.f, self.K, self.custs)
		self.assertAlmostEqual(tmp, -1)

	def test_UpperBd(self):
		tmp = DeltaPlus(self.tree, self.rev, [], 0, self.f, self.K, self.custs)
		self.assertAlmostEqual(tmp, -1)
	...
	def test_optRev(self):
		tmp = NPrev(self.tree, self.custs, self.rev, self.K, self.f, [])
		self.assertAlmostEqual(tmp, 0)

	def test_purchase(self):
		path   = self.tree.get_path(self.custs[0][0], self.custs[0][1])
...
class TestDPSingleton(unittest.TestCase):
	...
	def test_LowerBd(self):
		# Lower bound and upper bound should both be 1.5.
		tmp = DeltaMinus(self.tree, self.rev, [], 0, self.f, self.K, self.custs)
		self.assertAlmostEqual(tmp, 1.5)
	...
	def test_optRev(self):
		tmp = NPrev(self.tree, self.custs, self.rev, self.K, self.f, [0])
		self.assertAlmostEqual(tmp, 1)
...
class TestDPChain(unittest.TestCase):
	def setUp(self):
		...
		self.rev   = [6, 2.9, 3.2]
		self.K	   = [1.0, 1.0, 1.0]
		self.f     = [0.0, 0.4, 0.8, 1.2]
		self.custs = [[0, 2, 1.0/3], [1, 2, 1.0/3], [2, 2, 1.0/3]]

	def test_FirstStep(self):	
		# Optimal assortment when K = 0: add all three products 
		# 	Product 1 added because substitution cost > Rev(2) - Rev(1)
		SOpt1 = DP(self.tree, self.rev, np.zeros(3), self.f, self.custs)[0]
		self.assertEqual(sorted(SOpt1), [0, 1, 2])

	def test_LowerBd(self):
		# Lower bound when adding product 2
		tmp = DeltaMinus(self.tree, self.rev, [], 2, self.f, self.K, self.custs)
		self.assertAlmostEqual(tmp, 0.2)

	def test_UpperBd(self):
		# Upper bound when adding product 2
		tmp = DeltaPlus(self.tree, self.rev, [], 2, self.f, self.K, self.custs)
		self.assertAlmostEqual(tmp, 2.60)
	...
	def test_optRev(self):
		tmp = NPrev(self.tree, self.custs, self.rev, self.K, self.f, [0, 2])
